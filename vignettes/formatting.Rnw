...
df <- data.frame("Date" = Sys.Date()-0:4,
                 ...
                 "Percentage" = seq(-1, 1, length.out=5), stringsAsFactors = FALSE)
...
writeData(wb, 1, df, startRow = 16, startCol = 9, headerStyle = hs1,
          borders = "rows", borderStyle = "medium")

## to change the display text for a hyperlink column just write over those cells
writeData(wb, sheet = 1, x = paste("Hyperlink", 1:5), startRow = 17, startCol = 13)
