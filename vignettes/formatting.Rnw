...
df <- data.frame("Date" = Sys.Date()-0:4,
                 ...
                 "Percentage" = seq(0, 1, length.out=5), stringsAsFactors = FALSE)
...
writeData(wb, 1, df, startRow = 16, startCol = 2, headerStyle = hs1,
...
saveWorkbook(wb, "Formatting on write.xlsx", overwrite = TRUE)
\end{verbatim}
...
The conversion from POSIX to Excel datetimes uses format(Sys.time(), "%z")
to get the time zone offSet.  If POSIX values are not being written correct try setting
the timezone with (for example)
