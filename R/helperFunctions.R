...
#'   x =
#'     '=HYPERLINK("#Sheet2!B3", "Text to Display - Link to Sheet2")', startCol = 3
#' )
#' 
#' ## Internal - No text to display using makeHyperlinkString() function
#' writeFormula(wb, "Sheet1",
#'   startRow =
#'     1, x = makeHyperlinkString(sheet = "Sheet 3", row = 1, col = 2)
#' )
#' 
...
#'     sheet = "Sheet 3", row = 1, col =
#'       2, text = "Link to Sheet 3"
#'   )
#' )
#' 
...
#' writeFormula(wb, "Sheet1",
#'   startRow =
#'     4, x = makeHyperlinkString(
#'     sheet = "testing", row = 3, col =
#'       10, file = system.file("loadExample.xlsx", package = "openxlsx")
#'   )
#' )
#' 
...
#' writeFormula(wb, "Sheet1",
#'   startRow =
#'     3, x = makeHyperlinkString(
#'     sheet = "testing", row = 3, col =
#'       10, file = system.file("loadExample.xlsx", package = "openxlsx"), text = "Link to File."
#'   )
#' )
#' 
    ...
    if (stack) {
      for (i in 1:length(newStylesElements)) {
        wb$addStyle(
          sheet =
            sheet, style =
            newStylesElements[[i]]$style, rows =
            newStylesElements[[i]]$rows, cols = newStylesElements[[i]]$cols, stack = TRUE
        )
      }
    } else {
