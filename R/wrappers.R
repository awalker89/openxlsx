...
addFilter <- function(wb, sheet, rows, cols){
  ...
  ref <- paste(getCellRefs(data.frame("x" = c(rows, rows), "y" = c(min(cols), max(cols)))), collapse = ":")
  
  ## Removing existing filter if intercepts
  if(is.null(wb$worksheets[[sheet]]$autoFilter)){
    wb$worksheets[[sheet]]$autoFilter <- sprintf('<autoFilter ref="%s"/>', ref)
    names(wb$worksheets[[sheet]]$autoFilter) <- ref
  }else{
    
    refs <- names(wb$worksheets[[sheet]]$autoFilter)
    exFilters <- wb$worksheets[[sheet]]$autoFilter
    
    newRow <- rows
    newCols <- min(cols):max(cols)
      
    cols <- lapply(refs, function(rectCoords) convertFromExcelRef(unlist(regmatches(rectCoords, gregexpr("[A-Z]+", rectCoords)))))
    rows <- sapply(refs, function(rectCoords) as.numeric(unlist(regmatches(rectCoords, regexpr("[0-9]+", rectCoords)))))
    
    flag <- rows %in% newRow
    cols <- cols[flag]
    rows <- rows[flag]
    
    cols <- lapply(cols, function(x) seq(min(x), max(x)))
    toRemove <- sapply(cols, function(exCols) any(newCols %in% exCols))
    
    if(any(toRemove)){
      warning("Overwriting existing filters.")
      exFilters <- c(exFilters[flag][!toRemove], exFilters[!flag])
      refs <- c(refs[flag][!toRemove], refs[!flag])
    }
  
    wb$worksheets[[sheet]]$autoFilter <- c(exFilters, sprintf('<autoFilter ref="%s"/>', ref))
    names(wb$worksheets[[sheet]]$autoFilter) <- c(refs, ref)
